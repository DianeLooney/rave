...
func main() {
	...
	play(c, pitches.G3, 5*time.Second)
	...
	go func() {
		defer wg.Done()
		if err := play(c, pitches.G3, 5*time.Second); err != nil {
			panic(err)
		}
	}()
	...
	go func() {
		...
		if err := play(c, pitches.D4, 4*time.Second); err != nil {
			panic(err)
		}
	}()
	...
	go func() {
		...
		if err := play(c, pitches.B3, 3*time.Second); err != nil {
			panic(err)
		}
	}()
